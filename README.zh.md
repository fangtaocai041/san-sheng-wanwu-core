...
| 嵌套记忆层级 | ✅ | 热/温/冷三层 + 多层遗忘曲线 | Google Nested Learning 2025 |
| 递归思考框架 | ✅ | TRM 风格 think→act→observe 循环 | Jolicoeur-Martineau TRM 2025 |
| 资源分配策略 (6维) | ✅ | 事件驱动 → 行为倾向 | Damasio 1994, Sutton & Barto 1998 |
| MAGMA 四维图谱记忆 | ✅ | 语义/时序/因果/实体 + NetworkX | arXiv 2601.03236 |
| MoE 知识路由 | ✅ | 6专家查询级路由 | Kimi K2 2026, ICML 2026 |
| 12学科拓扑矩阵 | ✅ | 12×12 跨学科连接权重 | TopoNets ICLR 2025 |
| OCR 视觉感知 | ✅ | PaddleOCR-VL API / MCP | — |
| 12 学科知识图谱 | ✅ | 120+ 概念, 77 思想家 | — |
| 自我进化骨架 | ✅ | AST 分析 + 修改提案 | — |
| 双通道自愈引擎 | ✅ | 自发/诱发信号 + 平衡指标 | Pitt 2025 Science Advances |
| 愈愈发动机 | ✅ | 自愈+进化闭环: 检测→修复→验证→回滚 | — |
| 群体智能协议 | ✅ | Agent 间发现/消息/共识 | — |
| 5 层可解释性 | ✅ | L1 → L5 推理链 | — |
...
├── src/main.py       运行时主循环 (交互/守护/查询)
├── src/senses/       18个感知通道
├── src/cortex/       14个皮层模块
├── src/memory/       4层记忆 (MAGMA + SQLite)
├── src/motor/        运动输出
├── tests/            153项测试
├── config/           配置 + 价值观
├── docs/             知识储备 + 指南
...
pip install -e .
silicon-agent                    # 交互模式
silicon-agent --status           # 灵魂/情感/学习状态
```
